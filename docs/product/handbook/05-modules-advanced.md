...
挂在左侧 **滤镜调色** 工作区。预设滤镜与四滑条同一面板，不进【调整】。

### 1. 目标

非破坏调色：亮度/对比度/饱和度/色相写入 `filters`，Pixi `ColorMatrixFilter` 实时预览，可重置。预设滤镜是同一 `ColorAdjust` / 矩阵的快捷写入。
...
feat-011。P1。依赖 feat-009。入口：【滤镜调色】面板。

---

## 16b 人像（调研待定）

【人像】Tab 已占位。美颜、瘦脸、祛痘等**先不立项实现**，调研结论前不要写 OpenSpec change，也不要在 Pixi 内核里预埋美颜滤镜。跟踪项 **feat-034**。调研后再决定是哪一层：图层滤镜、独立会话，还是异步服务写回（可参考 feat-030）。
...
把主体从背景分离，并允许换背景。自动抠图与手动抠图都写回文档（新图层或 alpha/mask 字段），**不进 Pixi 内核当第二真源**。

### 2. 成熟产品

美图/手机修图：一键抠图 + 画笔补修 + 换背景。PS：选择与图层蒙版。本仓库：自动结果与手动修边缘都落在 Layer，换背景是新的底图层或替换 `background`，不是另开合成器。

### 3. 范围

| 级 | 内容 |
|---|---|
| MVP | 手动抠图（套索/画笔修边缘，写入 mask 或 alpha）；抠完可换纯色/图片背景 |
| P1 | 自动抠图（异步服务或本地算法，结果写回同一模型） |
| 暂不 | 视频抠像、云端登录素材当唯一背景源 |

与 feat-010 的矩形 crop、feat-024 的椭圆/Graphics mask **分开**：crop 是像素窗口；抠图是主体分离 + 换背景工作流。

### 4–9

会话在【抠图】面板；确认后一条 Command。自动抠图走「服务 → Pinia 图层」，与 feat-030 同一约束。scene 只同步 mask/底图。

### 13. 任务

`CUT-001` 抠图结果模型 · `CUT-002` 手动修边缘 · `CUT-003` 换纯色/图片背景 · `CUT-004` 自动抠图写回 · `CUT-005` History`

feat-033。P2。依赖 feat-009。入口：【抠图】面板。

---

## 16d 画笔（调研待定）

【画笔】Tab 已占位。多种画笔（硬边、软边、橡皮、马赛克等）**先调研**，不排进 P0/P1。跟踪项 **feat-035**。不要把 feat-013 的几何绘制当成画笔引擎。调研后再立 OpenSpec。
...
形状/矩形裁切可见区域。feat-010 的 crop 是矩形蒙版的特例；椭圆/alpha 蒙版走 feat-024。
...
同裁剪。P2。任务 `MSK-001` 起。feat-024，依赖 feat-010。
...
贴纸、图标、内置图形，以及在画布上画矩形/圆（几何见 04 章 feat-013）。同一工作区，两块入口：库置入 + 绘制工具。

【建议新增】P2（feat-026），几何绘制可先做 P1（feat-013）。
...
加载仍走 `Assets.load`。置入后就是 `ImageLayer` 或 `ShapeLayer`，不要为贴纸另起模型。

暂不实现云盘与登录。任务：`AST-001` 内置目录 · `AST-002` 置入为层 · `AST-003` 最近使用。入口：【素材】面板。
